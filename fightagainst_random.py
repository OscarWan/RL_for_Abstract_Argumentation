...
class Environment:
    ...
    def step(self, state, pool, node_dict, leaf_dict, is_train=True):
        ...
        agent_action_id = action_id_list[np.random.choice(len(action_id_list))] # random choice to move if multiple choices are allowed
...
if __name__ == "__main__":
    ...
    for structure in structures:
        ...
        # best_converged_winning_rate = [[],0,0,[],[]] # for different structures, best score should be clear
        # initialization
        ...
        plt.plot(episodes, [expected_winning_rate]*len(episodes), label='Ecpected winning rate')
        plt.title("random vs fight-against")
        plt.ylabel("winning rate for every 100 episodes")
        plt.xlabel("episodes")
        plt.legend()
        plt.grid()
        plt.savefig('/home/minyi/img/fight_against/random_fight_structure{0}.png'.format(structure))
        plt.show()
